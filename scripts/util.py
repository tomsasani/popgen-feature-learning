...
def check_for_missing_data(
    ...
    haps_with_zero = np.sum(batch[:, 0, :, :] == 0, axis=1)
    # figure out number of batches with 0-padded data
    batches_with_missing = np.any(haps_with_zero == H, axis=0)
...
def major_minor(matrix):
    ...
    assert derived_total.shape[0] == W

    # figure out where the derived sum is greater than half the haps
    switch_idxs = np.where(derived_total > (H / 2))[0]
    matrix[:, switch_idxs] = 1 - matrix[:, switch_idxs]

    # matrix[matrix == 0] = -1

    return matrix, switch_idxs


if __name__ == "__main__":

    img = np.ones((1, 3, 256, 32))
    img[:, 0, :, 3:9] = 0

    print (check_for_missing_data(img))
